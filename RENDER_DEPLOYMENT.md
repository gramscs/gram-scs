...
3. **No manual env setup required** - this repo includes a `render.yaml` blueprint that provisions and wires the database automatically
...
No manual environment variable entry is required for first deployment.
`render.yaml` provides:

```
FLASK_ENV=production
SECRET_KEY=<auto-generated by Render>
DATABASE_URL=<injected from Render managed PostgreSQL>
MASTER_DATABASE_URL=<injected from Render managed PostgreSQL>
...
For immediate deployment, the blueprint provisions a Render PostgreSQL instance and wires it to the app automatically.

1. Deploy with Blueprint.
2. Tables are created on startup (`AUTO_CREATE_TABLES=true`) for first-run readiness.
3. Verify connectivity via `/health/db` and functional routes like `/track`.
